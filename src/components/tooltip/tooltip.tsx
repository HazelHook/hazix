import { $, component$, Slot, useVisibleTask$, useId, useOnWindow, useSignal, useStylesScoped$ } from "@builder.io/qwik"
...
export const Tooltip = component$(({ content, position = "top", durationMs = 100, ...props }: TooltipProps) => {
	...
	const update = $(async () => {
		...
		if (triggerAnchor.value && tooltipAnchor.value && hasMouseEnterDebounced) {
			const { x, y } = await computePosition(triggerAnchor.value, tooltipAnchor.value as HTMLElement, {
				placement: position,
			})
	...
	return (
		<>
			...
			<Wrapper
				...
				role="tooltip"
				{...props}
				style={`--duration: ${durationMs}ms;--x: ${positionSignal.value.x || 0}px;--y: ${
					positionSignal.value.y || 0
				}px;`}
				data-state={stateSignal.value}
			>
				{content ? content : <Slot name="content" />}
			</Wrapper>
		</>
	)
})
