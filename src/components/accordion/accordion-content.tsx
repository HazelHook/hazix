...
export const AccordionContent = component$(({ ...props }: AccordionContentProps) => {
		...
		function getCalculatedHeight() {
			...
			contentChildren.forEach((element, index) => {
				...
				if (index === contentChildren.length - 1) {
					contentElement.style.setProperty("--qwikui-accordion-content-height", `${totalHeightSig.value}px`)
				}
			})
		}
	})
	...
	return (
		<div
			...
			style={{
				["--qwikui-collapsible-content-height" as string]: "var(--qwikui-accordion-content-height)",
				["--qwikui-collapsible-content-width" as string]: "var(--qwikui-accordion-content-width)",
			}}
			{...props}
		>
			<Slot />
