...
import { TabsContext, TabsStore } from "./tabs-context"
...
export const Tabs = component$((props: TabsProps) => {
	const behavior = props.activationMode ?? "manual"
	...
	const onTabKeyDown$ = $((key: KeyCode, currentTabId: string) => {
		...
		const currentFocusedTabIndex = enabledTabs.findIndex((tabPair) => tabPair.tabId === currentTabId)

		if ((!props.vertical && key === KeyCode.ArrowRight) || (props.vertical && key === KeyCode.ArrowDown)) {
		...
		if ((!props.vertical && key === KeyCode.ArrowLeft) || (props.vertical && key === KeyCode.ArrowUp)) {
	...
	const contextService: TabsStore = {
		selectTab$,
		isTabSelected$,
		isPanelSelected$,
		updateTabState$,
	...
	const selectEnabledIndex$ = $((index: number) => {
		let newIndex = index
		if (newIndex < 0) {
			newIndex = 0
		}
